...
feature {NONE, ES_TEST} -- implementation
	...
	remove (a_node: NODE [G])
		...
		do
			check (attached a_node.previous as p) and then (attached a_node.next as n) then
				l_pred := p
				l_succ := n
			end
...
feature -- commands
	...
	add_before (n: NODE [G]; e: detachable G)
		...
		do
			check (attached n.previous as prev) then
				add_between(e, prev, n)
			end
		ensure
	...
	add_after (n: NODE [G]; e: detachable G)
		...
		require
			node_exists:node_exists (n)
		do
			check (attached n.next as next) then
				add_between(e, n, next)
			end
		ensure
	...
	remove_at (i: INTEGER)
		...
		do
			...
			count_decremented_rf: count = old count - 1
			removed: not node_exists ((old node (i)))
		end
